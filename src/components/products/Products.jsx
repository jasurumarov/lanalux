...
const Products = () => {
    ...
    const sortedProducts = data?.products ? [...data.products].sort((a, b) => sortAsc ? a.price - b.price : b.price - a.price) : [];
    ...
    let product = sortedProducts?.map(el => (
        <div key={el.id} className="products__card">
            ...
            <p className='products__img-title' title={el.title}>{el.title}</p>
            <div className="products__card-oldPrice">
                <p><del>{Math.round(el.price * 10)}</del> <MdOutlineCurrencyRuble /></p>
                <h4>-10%</h4>
            </div>
            <h3>{el.price} <BiRuble /></h3>
            <div className="products__card-cart">
                    ...
                    <div className="category__filters-top">
                        ...
                        <div className='category__filters-top-filter'>
                            <div className="category__filters-top-filter__seemore">
                                <p>Показать:</p>
                                <button onClick={() => setSeeMore(1)} className={seeMore === 1 ? 'active' : ''}>8</button>
                                <button onClick={() => setSeeMore(3)} className={seeMore === 3 ? 'active' : ''}>/ 24</button>
                                <button onClick={() => setSeeMore(5)} className={seeMore === 5 ? 'active' : ''}>/ 40</button>
                            </div>
                            <div className="category__filters-top-filter__sena">
                                <p>По цене:</p>
                                <button onClick={() => setSortAsc(true)} className={sortAsc ? 'active' : ''}><LiaSortAmountDownAltSolid /></button>
                                <button onClick={() => setSortAsc(false)} className={!sortAsc ? 'active' : ''}><LiaSortAmountDownSolid /></button>
                            </div>
                        </div>
                    </div>
